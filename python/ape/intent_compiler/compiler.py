...
# CRITICAL: Includes fallback patterns for bare verbs and simple phrases.

SEMANTIC_PATTERNS: list[tuple[str, list[str], ActionCategory, float]] = [
    # =========================================================================
    # File Read — specific to general
    # =========================================================================

    # "read config.json" / "read the config.json"
    ...
    # "read the config file" / "read the file"
    (r"read\s+(the\s+)?([\w-]+\s+)?(file|document|config|configuration|contents?)",
     ["read_file"], ActionCategory.FILE_READ, 0.85),

    # "read the file" / "read the document"
    (r"read\s+(the\s+)?(file|document|contents?)",
     ["read_file"], ActionCategory.FILE_READ, 0.80),

    # "show me the file" / "show the contents"
    ...
    # "get the contents of config.json"
    ...
    # FALLBACK: bare "read" or "read something" (catch-all for read intent)
    (r"\bread\b",
     ["read_file"], ActionCategory.FILE_READ, 0.65),
    ...
    # FALLBACK: bare "list" → list_directory
    (r"\blist\b",
     ["list_directory"], ActionCategory.FILE_READ, 0.6),
    ...
    # FALLBACK: "find" / "search"
    (r"\bfind\b",
     ["search_files"], ActionCategory.FILE_READ, 0.55),
    (r"\bsearch\b",
     ["search_files"], ActionCategory.FILE_READ, 0.55),
    ...
    (r"write\s+a\s+(file|document|report)",
     ["write_file"], ActionCategory.FILE_WRITE, 0.8),
    (r"edit\s+(the\s+)?[\w./\\-]+",
     ["write_file"], ActionCategory.FILE_WRITE, 0.75),
    (r"modify\s+(the\s+)?[\w./\\-]+",
     ["write_file"], ActionCategory.FILE_WRITE, 0.75),

    # FALLBACK: bare "write" / "save" / "create" / "update" / "edit" / "modify"
    (r"\bwrite\b",
     ["write_file"], ActionCategory.FILE_WRITE, 0.6),
    (r"\bsave\b",
     ["write_file"], ActionCategory.FILE_WRITE, 0.55),
    (r"\bcreate\b",
     ["write_file"], ActionCategory.FILE_WRITE, 0.55),
    (r"\bupdate\b",
     ["write_file"], ActionCategory.FILE_WRITE, 0.55),
    ...
    # "delete temp files" / "delete the files"
    (r"delete\s+(the\s+)?(temp\s+)?(files?|documents?)",
     ["delete_file"], ActionCategory.FILE_DELETE, 0.8),
    (r"remove\s+(the\s+)?(temp\s+)?(files?|documents?)",
     ["delete_file"], ActionCategory.FILE_DELETE, 0.8),
    (r"clean\s*up\s+(the\s+)?(temp\s+)?(files?|data)",
     ["delete_file"], ActionCategory.FILE_DELETE, 0.75),

    # FALLBACK: bare "delete" / "remove"
    (r"\bdelete\b",
     ["delete_file"], ActionCategory.FILE_DELETE, 0.6),
    (r"\bremove\b",
     ["delete_file"], ActionCategory.FILE_DELETE, 0.55),
    ...
    # FALLBACK: bare "fetch" / "download"
    (r"\bfetch\b",
     ["http_get"], ActionCategory.NETWORK, 0.55),
    (r"\bdownload\b",
     ["http_get"], ActionCategory.NETWORK, 0.55),
    ...
    # FALLBACK: bare "query"
    (r"\bquery\b",
     ["query_data"], ActionCategory.DATABASE_READ, 0.55),
    ...
    # FALLBACK: bare "run" / "execute"
    (r"\brun\b",
     ["execute_code"], ActionCategory.COMPUTE, 0.5),
    (r"\bexecute\b",
     ["execute_code"], ActionCategory.COMPUTE, 0.5),
]
...
class IntentCompiler:
    ...
    def compile(
        ...
        This is the primary API of the IntentCompiler.
        ...
        if not signals:
            raise IntentAmbiguityError(
                "Could not extract any action signals from prompt",
                prompt=prompt
            )
        ...
        if total_allowed == 0:
            raise IntentNarrowingError(
                "Policy narrowing removed all actions",
